...
router.post("/chat", async (req, res) => {
  ...
  try {
    ...
    const { event, location, clothing, time, nudity, emotion, dynamic, num } =
      trimObjectStrings(req.body);

    let message = `Event: ${event}, Location: ${location}, Clothing: ${clothing}, Time: ${time}, Nudity: ${nudity}, Emotion: ${emotion}, Dynamic: ${dynamic}, Number of PPVs: ${num}`;
    ...
    do {
      if (numChecks >= maxChecks) {
        throw new Error(
          "Max number of checks reached, run status check aborted."
        );
      }
      const getRunStatusResponse = await axios.get(
    ...
    const threadResponse =
      getMessagesResponse.data.data[0].content[0].text.value;

    // Return the response back to the invoking API call
    res.json({ result: threadResponse });
  } catch (error) {
